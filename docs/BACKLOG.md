...
- **記帳者的**（他先墊了，家族欠他）⇒ 需要「應收／應付」的概念，也就是這個 app
  **刻意沒做**的分帳
- **這個圈共同的**（大家都在花這筆）⇒ 只是換一個 `ledgerId`

兩者會長出**完全不同的資料模型**。想清楚這個之前不要寫任何程式。

## 五、可驗證的信號只有一個：統一發票

全 app 只有發票號碼有外部背書（財政部發的、唯一的）：記錄可以被**遺漏**，但無法被**捏造**。
「帳本中有發票背書的比例」是真信號，遠強過手動輸入。

但它有偏誤——現金、小攤販、傳統市場全部不算，而那正是長輩的主要消費形態。
⇒ **可以當輔助資訊，絕不可當評分。**（也見原則一：任何評分都會變成脅迫的槓桿。）
